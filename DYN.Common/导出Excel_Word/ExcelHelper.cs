            ...
            try
            {
                if (data != null && data.Rows.Count > 0)
                {
                    ...
                    foreach (System.Data.DataRow row in data.Rows)
                    {
                        ...
                        foreach (System.Data.DataColumn column in data.Columns)
                        {
                            if (Regex.IsMatch(row[column].ToString().Trim(), @"^\d{12,}$") || Regex.IsMatch(row[column].ToString().Trim(), @"^\d+[-]\d+$"))
                            {
                                sbHtml.Append("<td style=\"vnd.ms-excel.numberformat:@\">").Append(row[column].ToString()).Append("</td>"); ;
                            }
                            else
                            {
