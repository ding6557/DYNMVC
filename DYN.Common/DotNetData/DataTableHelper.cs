...
namespace Maticsoft.Common
{
    ...
    public class DataTableHelper
    {
        ...
        /// 删除数据表中不需要的列
        /// </summary>
        /// <param name="needFiledsList">需要保留的字段列表，如 "|acceptedfiletype|isicrfile|icrformsourceid";</param>
        /// <param name="dt">源数据表</param>
        /// <returns></returns>
        public static DataTable RemoveColums(string needFiledsList, DataTable dt)
        {

            needFiledsList = needFiledsList.ToLower();
            string delFiledsList = "";
            for (int i = 0; i < dt.Columns.Count; i++)
            {
                if (needFiledsList.IndexOf(dt.Columns[i].Caption.ToLower()) == -1)
                {
                    delFiledsList += dt.Columns[i].Caption + ",";
                }
            }
            string[] delFeilds = delFiledsList.Split(',');

            for (int i = 0; i < delFeilds.Length; i++)
            {
                if (delFeilds[i] != "")
                {
                    dt.Columns.Remove(delFeilds[i]);
                }
            }
