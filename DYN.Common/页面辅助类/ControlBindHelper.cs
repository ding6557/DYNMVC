...
namespace Maticsoft.Common
{
    ...
    public class ControlBindHelper
    {
        ...
        /// <param name="_ID">绑定字段主键</param>
        /// <param name="_Memo">默认显示值</param>
        public static void BindHtmlSelect(DataTable dt, HtmlSelect select, string _Name, string _ID, string _Memo_text,string _Memo_value)
        {
            select.DataSource = dt;
            select.DataTextField = _Name;
            select.DataValueField = _ID;
            select.Attributes.Add("title", "htmlselect");
            select.DataBind();
            if (!string.IsNullOrEmpty(_Memo_text.Trim()))
            {
                select.Items.Insert( 0 , new ListItem(_Memo_text, _Memo_value));
            }
        }
        /// <summary>
